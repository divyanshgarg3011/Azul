...
simple_campaign_bp = Blueprint('simple_campaign', __name__)
...
@simple_campaign_bp.route('/generate-campaign-basic', methods=['POST'])
@cross_origin()
def generate_campaign_basic():
    """Basic campaign generation without cloud dependencies"""
    try:
        ...
        response_data = {
            'success': True,
            'mode': 'basic',
            'note': 'Generated using basic analysis. Full AI features available with DashScope API configuration.',
...
@simple_campaign_bp.route('/test-basic', methods=['GET'])
@cross_origin()
def test_basic():
